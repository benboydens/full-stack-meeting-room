...
try {
    ...
    const bookings_col = database.collection("bookings");

    // this option prevents additional documents from being inserted if one fails
    const options = { ordered: true };
    const result = await bookings_col.insertMany(all_bookings, options);
    ...
    // parsing to JSON object
    const all_rooms = JSON.parse(data);

    // getting collection bookings
    const collection = database.collection("rooms");

    // this option prevents additional documents from being inserted if one fails
    const result2 = await collection.insertMany(all_rooms, options);
